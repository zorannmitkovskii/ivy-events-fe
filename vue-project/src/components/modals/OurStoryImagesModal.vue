<template>
  <BaseModal :open="open" :title="t('ourStory.images.title')" @close="emit('close')">
    <div class="images-gallery">
      <!-- Existing image -->
      <div v-if="currentImageUrl" class="image-item">
        <img :src="currentImageUrl" alt="Story image" class="gallery-image" />
        <button type="button" class="remove-btn" @click="removeImage" :disabled="uploading">
    ...
    <div v-if="pendingFiles.length" class="pending-section">
      <div class="pending-label">{{ t("ourStory.images.pendingFiles", { count: pendingFiles.length }) }}</div>
...
watch(
  ...
  (v) => {
    if (v && props.item) {
      currentImageUrl.value = props.item.imageUrl || "";
      pendingFiles.value = [];
      errorMsg.value = "";
      revokeAll();
    }
...
async function uploadAll() {
  if (!props.item?.id || !pendingFiles.value.length) return;

  uploading.value = true;
  errorMsg.value = "";

  let lastUrl = currentImageUrl.value;

  try {
    const files = pendingFiles.value.map((pf) => pf.file);
    const results = await invitationImagesService.uploadOurStoryImages(onboardingStore.eventId, files);
    if (Array.isArray(results) && results.length) {
      lastUrl = results[results.length - 1]?.url || lastUrl;
    }

    // Extract S3 key and persist on the OurStory item
    const marker = "/public/images/";
    const idx = lastUrl.indexOf(marker);
    const s3Key = idx !== -1 ? lastUrl.substring(idx + marker.length) : lastUrl;
    await ourStoryApi.update(props.item.id, { imageUrl: s3Key });

    currentImageUrl.value = lastUrl;
    pendingFiles.value = [];
    revokeAll();
